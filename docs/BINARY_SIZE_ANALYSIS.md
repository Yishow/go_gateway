...
- **優化前 gateway.exe**: 28.89 MB
- **優化後（使用 `-s -w -trimpath`）**: 20.34 MB ✅ **已實作**
- **實際減少**: 8.55 MB（約 29.6%）
...
| 優化項目              | 預期減少    | 實際減少     | 累計大小     | 狀態    |
| --------------------- | ----------- | ------------ | ------------ | ------- |
| 當前大小              | -           | -            | 28.89 MB     | ✅ 基準 |
| 移除除錯資訊（-s -w） | -8.52 MB    | **-8.55 MB** | **20.34 MB** | ✅ 完成 |
| 依賴優化              | -2~4 MB     | -            | 16~18 MB     | ⏳ 待做 |
| 前端優化              | -0.1~0.3 MB | -            | 15.9~17.9 MB | ⏳ 待做 |
| UPX 壓縮（可選）      | -5~7 MB     | -            | 10~12 MB     | ⏳ 可選 |

**當前狀態**：✅ **已優化至 20.34 MB**（減少 29.6%）

**最終預期**：約 **15-18 MB**（完成依賴優化後，不使用 UPX）或 **10-12 MB**（使用 UPX）
...
已修改 `start.ps1` 和 `build.ps1`，加入 `-s -w -trimpath` 標誌。
