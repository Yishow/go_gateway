...
2. `/studio/runtime`
   - 定位：setup 後 focused runtime observer
   - 決策：與 V2 一起維持與驗收
3. `/test`
   - 定位：工程測試工具
   - 決策：先暫停，後續再完善
...
5. `/studio`
   - 定位：已退場的 legacy 完整工作台
   - 決策：owner 已授權立即刪除 dedicated route；刪除後走 generic unknown-route policy
...
1. [Legacy `/studio` pre-delete inventory](./studio-mainline.md)
...
6. [Legacy deletion release record](../../releases/retire-legacy-studio-and-polish-v2.md)
7. [缺口與 change roadmap](./gap-roadmap.md)

## Surface 總表

| Surface | Route | 主要受眾 | 目前定位 | 主要資料真相 | 整合成熟度 | 目前策略 |
| --- | --- | --- | --- | --- | --- |
| Legacy Studio Mainline | `/studio` | 歷史進階操作者、維運人員 | pre-delete inventory；不再是產品入口 | `datalink` CRUD / review / output APIs（歷史 owner） | 歷史完整、現行不保證 | 立即刪除 |
...
- `/studio/v2` 是網站服務唯一產品 setup 入口；`/studio/runtime`、`/test`、`/gateway/*` 保留各自 route identity。
- `/studio` 的完整功能與 API 盤點只作為 pre-delete evidence；它不再是 fallback，也不應由新 route、handler 或 redirect 重新引入。
...
- 若任務只涉及 `/studio/v2` 或 `/studio/runtime`，預設先看 `studio-v2-runtime.md`；若任務涉及 legacy deletion，先看 `START_HERE.md`、`context.json`、`CURRENT_STATE.md` 與 release record，再把 `studio-mainline.md` 當歷史 inventory。
