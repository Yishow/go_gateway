...
## 臨時任務：2026-04-09 Studio 主線重定義 brainstorm（本輪）

- [in_progress] 重新定義 `/studio` 主產品主線為 `Device -> Source -> Tag -> Database`
- [pending] 將 `Local Modbus` / `MQTT` 從主線 step ownership 拆出，重定義為 `Share / Publish` 附屬能力
- [pending] 決定 `Share / Publish` 在 `/studio` 內的 IA 位置、進入時機、與主線 handoff 方式
- [pending] 產出適合 Google Stitch / Pencil MCP 的頁面 prompt 與 layout guidance
- [pending] 評估此 brainstorm 流程如何產品化為可重複使用的 skill

**備註**：
- 使用者已明確確認：專案主要目的為「取資料存 DB」；`Local Modbus` 與 `MQTT` 屬資料分享能力，不應再作為主線 step 完成條件。
- 目前暫定建議方向：
  - 主線：`Device -> Source -> Tag -> Database`
  - 附屬能力：`Share / Publish`
  - IA 建議：保留在 `/studio` 內作為主線完成後可進入的附屬工作區，而非與主線 step 並列。
