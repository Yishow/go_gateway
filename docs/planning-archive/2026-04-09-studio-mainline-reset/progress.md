...
#### Session 29: Studio 主線重定義 brainstorm 進行中（本輪）
- ✅ 已重新盤點 `App.tsx`、`/studio` workbench shell、Output target switch、相關 OpenSpec 與 planning records。
- ✅ 已確認現行 product / spec 模型仍把 Output 定義為 unified workspace：`Local Modbus + Database` 共用同一個 Step 4。
- ✅ 使用者已明確改寫產品目標：
  - 主線應為 `Device -> Source -> Tag -> Database`
  - `Local Modbus` / `MQTT` 為資料分享能力，屬附屬工作區，不應再綁在主線 step 完成條件中
- ✅ 目前 IA 初步建議：
  - 保留 `/studio` 作為主產品主線
  - 主線完成於 `Database`
  - 另在 `/studio` 內承接 `Share / Publish` 附屬能力，而不是與主線 step 並列
- ⏭️ 下一步：確認 `Share / Publish` 的 IA 位置與進入方式，再產出適合 Google Stitch / Pencil MCP 的 prompt 結構
