...
## 2026-04-09 Studio 主線重定義 brainstorm 新發現
- 使用者明確重新定義產品主線：專案主要目的不是「把 tag 發到多種 output」，而是 **從 PLC 取資料並落地到 Database**。
- 因此現行 `Output = Local Modbus + Database` 的 unified step model 已不再符合產品心智；它把「主資料落地」與「附屬資料分享」混成同一個完成條件。
- `Local Modbus` 與 `MQTT` 在新定義下屬於 **Share / Publish capability**，本質是把已整理好的資料再分享給其他設備或系統，而不是主線必經步驟。
- 對 IA 的初步判斷：
  - 不建議維持現況，因為會繼續讓主線被附屬能力污染。
  - 不建議完全切離 `/studio`，因為會斷掉從 tag / database handoff 到分享能力的上下文。
  - 目前最佳候選是：`/studio` 主線收斂到 `Database`，並在主線完成後提供 `Share / Publish` 附屬工作區承接 `Local Modbus` / `MQTT`。
- 這個方向與現行 OpenSpec 存在明顯衝突：目前 spec 仍把 Output 定義為同一個 `/studio` step 內的 unified workspace，且要求 `Local Modbus` / `Database` 同步可見；若採新模型，後續需要 spec amendment，而不是只改 layout。
