...
func (h *TestHandler) createClientWithDebug(protocol string, config map[string]interface{}, connectionID string) (interface{}, error) {
	...
	getDuration := func(def time.Duration) time.Duration {
	...
	case "modbus_tcp":
		...
		unitID, err := testModbusUnitID(config)
		if err != nil {
			return nil, err
		}
		timeout := getDuration(2000 * time.Millisecond)
	...
	case "fatek_serial":
		...
		station := getInt("station", 1)
		timeout := getDuration(1000 * time.Millisecond)
