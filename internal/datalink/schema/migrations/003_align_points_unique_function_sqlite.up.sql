-- SQLite migration: align points uniqueness from (device_id, address) to (device_id, address, function)

PRAGMA foreign_keys=OFF;
BEGIN TRANSACTION;

CREATE TABLE points_v11 (
    id              TEXT PRIMARY KEY,
...
SELECT
    id, device_id, name, description, address, COALESCE(function, ''), data_type, mode,
    polling_group_id, last_read_at, last_value, last_error, enabled, created_at, updated_at
FROM points;

DROP TABLE points;
ALTER TABLE points_v11 RENAME TO points;
