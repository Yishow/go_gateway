...
- `scanMapping` 與 `scanMappings` 兩條路徑都必須一起補 `sql.NullString`；只修單列或多列其中一條，`GetByID` 與 `List` 仍會留下半套脆弱行為。
- backend blocker 一旦解除，原本 Phase 6 的 shared `/studio` fixture 立即恢復：同一個 `Tag -> Output` handoff 能重新載入 Local Modbus / Database incident workspace，代表這不是版本專屬契約問題，而是 shared data scan 問題。
- blocker 移除後，本輪 follow-up 不需要再做額外 wrapper polish；新的 browser evidence 仍然讀得出 `v2` 的 incident-desk identity，沒有被 calm copy 拉向 `v1` control-room 語氣。
...
## 2026-04-09 Phase 6 v2 最終整體驗收新發現
- v2 到了 final E2E 仍然保持最穩定的 **incident-desk canonical identity**：從首屏 `SHELL 事件桌` 到 `事件桌 — 來源`、Tag handoff、`OUTPUT INCIDENT TARGET`，主操作骨架與 blocker language 都是一套連續語言，而不是各 phase 各自長成不同的 page。
- 在同一條 `/studio` 主線上，v2 的最短路徑仍然最清楚：shell 先直接講 `2/3 steps ready · 1 need attention`，接著 `打開輸出工作區` 把 operator 直接送回當前 blocker phase；這個 shell -> step-local handoff 比 baseline 更短，也比 calmer 的 control-room 語氣更直接。
- v2 Output 的 final validation 再次證明它不是只在 Local Modbus 成立：切到 Database target 後，incident desk 會把 command panel 改成 connector / validation / schema 方向，而 shared workboard 仍保持同一個 mapping contract。這代表 `v2` 在兩個 target families 上都維持了同一條版本語言。
- 本輪 final validation 抓到一個直接耦合的 a11y 缺口：`DatabaseTargetBoard.tsx` 的 database mapping selects 缺少 `name` 屬性。這不是 compare 額外挑毛病，而是 v2 自己在 final E2E 中暴露出的表單可辨識缺口；已在封帳前補上，reload 後 DOM 掃描回到 `0` 個缺 `id/name` 的 form field issue。
- 本輪正式 evidence（`agent-browser`，session artifacts）：
...
## 2026-04-09 Phase 5 v2 Sentry Incident Desk 新發現
- `v2` Phase 5 成立的關鍵不是把 shell 做得更滿，而是做出一個真正的 **incident strip**：在 command strip 內同時講 readiness、active blocker、diagnostics refresh 與 shortest return action，讓 operator 不用先鑽進 step-local workspace 才知道目前主線卡在哪裡。
- 這次 shell 不是吃掉原本 flow，而是 **疊加在 shared flow 上面**：真實頁面上仍同時保留既有 primary CTA（例如 `Go to source`）與 incident strip 的 `Open output workspace`；這證明 v2 新增的是 diagnostics / return layer，而不是默默改寫 step-local ownership。
- 在真實裝置 `UI 4.3 Modbus TCP` 上，incident strip 會直接露出：
  - `2/3 steps ready · 1 need attention`
  - `Output setup is still pending. Review Local Modbus or Database output and apply the mapping.`
  - `Shell diagnostics refreshed`
  - `Open output workspace`
  這比 baseline 只有 generic CTA、沒有 shell-level blocker summary 的情況明顯更接近 canonical shell / diagnostics flow。
- refresh affordance 進到 shell 後，compare 的關鍵差異變得很明確：baseline 把 refresh / validation 留在 step-local surface；`v2` 則把「先刷新 shell diagnostics，再沿最短路徑回主線」做成一級操作語言。
- 本輪正式 evidence（`agent-browser`，session artifacts）：
  - `v2-shell-overview.png`
  - `v2-shell-refresh-success.png`
  - `v2-shell-return-to-output.png`

## 2026-04-09 Phase 4 v2 Sentry Incident Desk 新發現
- `v2` Output 這輪真正成立的關鍵不是再 fork 一套 Output board，而是把 shared `LocalModbusBoard` / `DatabaseTargetBoard` 降成右側 workboard，再由 `MuiOutputIncidentDesk` 把 target diagnosis、rule-scoped summary 與 recovery handoff 提到左側 command dock。這樣才能同時守住同一個 `/studio` route 與 same-domain flow。
- command dock 如果直接沿用 shared board 的按鈕文案，會立刻和既有 output step 測試 / a11y query 衝突；因此 v2 需要 **desk-specific action labels**（例如 `Desk · run dry-run`、`Desk · open connector setup`），才能既保留 click-through，又不破壞 shared contract。
- 在真實裝置 `UI 4.3 Modbus TCP` 上，從 Tag step handoff 回 Output 後，v2 已能讀到真正的 rule-scoped revision（`82b8eea1-0876-4284-a9d7-e15db8779ca3`）與兩個 target families 的候選集合，不再只停留在「global tag chips 可操作、但 rule scope 為空」的 baseline 雙重訊號。
- Local Modbus path 的新價值不只是外觀：left desk 可直接觸發 shared `dry-run`，且真實 blocker `port 5020 is already in use` 仍留在同一條 shared runtime path 上；v2 做的是 diagnosis / command zoning，不是替換 shared runtime contract。
- Database path 的新價值同樣在 command semantics：切到 Database 後，left desk 立即改成 connector-oriented commands；用真實 empty connector save 可直接再現 `Enter a connector name first` blocker，證明 v2 已把 Database 與 Local Modbus 的 command language 分開，而不是只共用同一套 CTA。
- 本輪正式 evidence（`agent-browser`，session artifacts）：
