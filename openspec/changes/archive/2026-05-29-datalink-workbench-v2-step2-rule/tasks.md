...
- [x] 5.5 跑 `bash scripts/check_file_lines.sh`，確認 step2 所有新檔 ≤ 300 行。
- [x] 5.6 手動 smoke：完整跑「新增第二條規則 → 改 count → Shift+click skip 範圍 → Ctrl+click 多選 → 批次略過選取 → 開 share → 手動改 share 起點 → 看合併表衝突高亮 → 點繼續到 Step 3 placeholder」。
