...
- [x] 1.1 Deliver Runtime diagnostics expose collector to database delivery stages by defining a stage-oriented diagnostics model over collector, mapping, runtime projection, and database delivery, and verify it with backend tests that assert the failing stage is reported for broken flows.
- [x] 1.2 Deliver Runtime diagnostics keep recent success and failure context under Diagnostics use a stage-oriented model by recording latest success and failure timestamps and reasons for selected scope, and verify it with runtime or writer tests that update diagnostics on both success and failure paths.
