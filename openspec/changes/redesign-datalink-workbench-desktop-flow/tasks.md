...
- [x] 1.1 Replace the current workbench desktop frame with the approved five-region shell (`StepRail`, `ContextBar`, `PrimaryWorkArea`, `InspectorPanel`, `BottomSummaryBar`).
- [x] 1.2 Move page-level summary/readiness behavior out of `ActionDock` and into shared inspector/bottom-bar state.
- [x] 1.3 Extend workbench state/selectors to support shared readiness, active output target, Step 2 rule metadata, and cross-step traceability.

## 2. Device workspace redesign

- [x] 2.1 Build the new `DeviceWorkspace` browser surface with search, filters, create, refresh, and clone entry points.
- [x] 2.2 Implement the Step 1 inspector and context-bar capability summary (`address base`, `word order`, `unit id`, protocol traits, recent test history).
- [x] 2.3 Add the workbench-specific clone flow and cover Step 1 desktop interactions with tests.
...
- [x] 5.1 Create the shared `OutputCandidateBoard` and output readiness/filter model for Local Modbus and Database targets.
- [x] 5.2 Build the Local Modbus studio with register map visualization, base/offset context, auto-map strategies, dry-run/test write, health summary, and sync-result feedback.
- [x] 5.3 Build the Database studio with schema snapshot, type badges, required/missing highlighting, write-row preview, and sync-result feedback.
- [x] 5.4 Build the Step 4 inspector trace for source -> tag -> output status and target-specific blocked reasons.
