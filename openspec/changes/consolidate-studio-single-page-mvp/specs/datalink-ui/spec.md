...
The UI SHALL provide a guided workflow that keeps the complete datalink product flow inside `/studio` as the primary workbench for **Device -> SourceRule -> Tag review -> Output review/apply**.
...
8. The system SHALL allow operators to start from a separate setup entry page before entering the workbench, but direct access to `/studio` SHALL remain available.
...
#### Scenario: Setup entry page can launch the existing workbench flow
- **WHEN** an operator starts from the setup entry page and chooses to enter the workbench
- **THEN** the system routes the operator into the existing `/studio` workflow
- **AND** direct access to `/studio` remains available for operators who skip the entry page
