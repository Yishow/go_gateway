...
- [x] 2.1 實作「分頁導覽 + 已選設備條」同列水平布局（桌面）
- [x] 2.2 補齊平板/手機斷點行為（改為上下堆疊）
- [x] 2.3 以 context bar 顯示設備名稱、協議、狀態與切換入口
- [x] 2.4 context bar 顯示最近切換時間
- [x] 2.5 設備狀態色階（active/offline/read-only）與對應文案
- [x] 2.6 搜尋列與上方控制列採同一視覺網格對齊
- [x] 2.7 無設備空狀態僅提供「選擇設備」與「新增設備」兩個主要行動
- [x] 2.8 右側 Quick Actions 依設備狀態動態排序
