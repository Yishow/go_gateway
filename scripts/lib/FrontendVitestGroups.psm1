...
function Invoke-FrontendVitestGroupChildRun {
    ...
    $result = Invoke-FrontendVitestOwnedProcess -RepoRoot $RepoRoot -ArgumentList (Get-FrontendVitestGroupCommandArguments -Mode $Mode -Files $Files -RepoRoot $RepoRoot) -WatchdogSeconds $WatchdogSeconds
    return $result
}
...
function Invoke-FrontendVitestGroup {
    ...
    $records = [System.Collections.Generic.List[object]]::new(); $artifactIndex = [System.Collections.Generic.List[object]]::new(); $recordPaths = [System.Collections.Generic.List[string]]::new(); $hardFailure = $false; $blockedFailure = $false
    for ($repeatIndex = 1; $repeatIndex -le $Repeat; $repeatIndex++) {
        ...
        if ($childStartError -ne "" -or $childRuntimeError -ne "" -or $childCleanupOutcome -eq "unsafe-termination-refused" -or $record.result -ne "pass" -or $record.classification -ne "pass") { $hardFailure = $true; if ($record.result -eq "blocked" -or $childStartError -ne "" -or $childRuntimeError -ne "" -or $childCleanupOutcome -eq "unsafe-termination-refused") { $blockedFailure = $true } }
    }
    ...
    if ($hardFailure) { throw "$Mode evidence is $($summary.result); summary preserved at $summaryPathOut" }
}
