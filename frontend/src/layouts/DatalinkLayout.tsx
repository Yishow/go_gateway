...
export default function DatalinkLayout() {
  ...
  const navItems = [
    // --- 系統總覽 ---
    { path: '/datalink', label: t('nav.dashboard'), icon: (
      <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z" />
      </svg>
    ), group: 'overview' },
    
    // --- 資料收集 (Data Collection Pipeline) ---
    { path: '/datalink/devices', label: t('nav.devices'), icon: (
      <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 3v2m6-2v2M9 19v2m6-2v2M5 9H3m2 6H3m18-6h-2m2 6h-2M7 19h10a2 2 0 002-2V7a2 2 0 00-2-2H7a2 2 0 00-2 2v10a2 2 0 002 2zM9 9h6v6H9V9z" />
      </svg>
    ), group: 'collection' },
    { path: '/datalink/points', label: t('nav.points'), icon: (
      <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />
      </svg>
    ), group: 'collection' },
    { path: '/datalink/polling-groups', label: t('nav.pollingGroups'), icon: (
      <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    { path: '/datalink/tags', label: t('nav.tags'), icon: (
      <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z" />
      </svg>
    ), group: 'processing' },
    { path: '/datalink/mappings', label: t('nav.mappings'), icon: (
      <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
         <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4" />
      </svg>
    ), group: 'processing' },
    { path: '/datalink/wizard', label: t('nav.mappingWizard'), icon: (
      <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    // --- 系統設定 ---
    { path: '/datalink/settings', label: t('nav.settings'), icon: (
      <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    // --- 開發工具 ---
    { path: '/test', label: t('nav.protocolTestTool'), icon: (
      <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
         ...
         <div className="absolute bottom-6 left-6 right-6">
            <div className="bg-slate-800/50 backdrop-blur-sm rounded-xl p-4 border border-slate-700/50">
               <div className="flex items-center space-x-3">
                   ...
                   <div>
                       <div className="text-xs font-medium text-slate-300">{t('system.systemAdmin')}</div>
                       <div className="text-[10px] text-slate-500">{t('system.version')}</div>
                   </div>
               </div>
            </div>
         </div>
             ...
             <div>
                 ...
                 <div className="text-sm text-slate-500 flex items-center gap-2">
                     ...
                     <span className="text-slate-200 capitalize">
                        {location.pathname === '/datalink' ? t('nav.dashboard') : location.pathname.split('/').pop()}
                     </span>
                 </div>
             </div>
