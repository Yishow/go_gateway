...
import { useSmartDashboardShortcuts } from '../useKeyboardShortcuts';
...
describe('useSmartDashboardShortcuts', () => {
  it('includes flow action shortcuts when handlers are provided', () => {
    ...
    expect(shortcuts.some((s) => s.key === 'Enter' && s.ctrl && !s.shift)).toBe(true);
    expect(shortcuts.some((s) => s.key === 'Enter' && s.ctrl && s.shift)).toBe(true);
    expect(shortcuts.some((s) => s.key.toLowerCase() === 'r' && s.alt)).toBe(true);
  });
  ...
  it('does not include recover shortcut when recover handler is absent', () => {
    ...
    expect(shortcuts.some((s) => s.key.toLowerCase() === 'r' && s.alt)).toBe(false);
  });
});
