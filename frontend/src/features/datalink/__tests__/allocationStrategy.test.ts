...
import { findNearestValidContiguousSpan } from '../allocationStrategy';
...
describe('findNearestValidContiguousSpan', () => {
  ...
  it('returns no_contiguous_span when no valid span exists in window', () => {
    const blocked = new Set(Array.from({ length: 10 }).map((_, index) => String(40001 + index)));
    const result = findNearestValidContiguousSpan({
      startAddress: '40001',
      spanSize: 2,
      protocol: 'modbus_tcp',
    ...
    expect(result).toEqual({ startAddress: null, reason: 'no_contiguous_span' });
  });
