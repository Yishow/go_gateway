...
describe('commitLifecycle', () => {
  it('validates commit gates before execution', () => {
    expect(canExecuteCommit({ canActivate: false, mappingEnabled: true })).toEqual({
      ok: false,
      reason: 'not_validated',
    });
    expect(canExecuteCommit({ canActivate: true, mappingEnabled: false })).toEqual({
      ok: false,
      reason: 'mapping_disabled',
    });
    expect(canExecuteCommit({ canActivate: true, mappingEnabled: true })).toEqual({
      ok: true,
      reason: 'ok',
    });
  });
