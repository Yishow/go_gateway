...
describe('MemoryGrid', () => {
  ...
  it('should render float32 planned as one logical cell per allocation', () => {
    ...
    const cells = screen.getAllByTestId('grid-cell');
    const float32Blocks = cells.filter(
      (el) => el.getAttribute('data-span') === '2'
    );
  ...
  it('should render int64 planned as one logical cell per 4-address group', () => {
    ...
    const cells = screen.getAllByTestId('grid-cell');
    const span4Blocks = cells.filter(
      (el) => el.getAttribute('data-span') === '4'
    );
  ...
  it('should render cells as keyboard-focusable buttons', () => {
    ...
    expect(cells[0]).toHaveAttribute(
      'aria-label',
      expect.stringContaining('Address 40001')
    );
  });
});
