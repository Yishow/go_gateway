...
describe('useHistory', () => {
  ...
  it('should undo action', () => {
    ...
    expect((undoneAction as HistoryAction<string> | null)?.data).toBe(
      'data-1'
    );
    expect(result.current.currentIndex).toBe(-1);
    expect(result.current.canUndo).toBe(false);
  ...
  it('should redo action', () => {
    ...
    expect((redoneAction as HistoryAction<string> | null)?.data).toBe(
      'data-1'
    );
    expect(result.current.currentIndex).toBe(0);
    expect(result.current.canUndo).toBe(true);
...
describe('usePointHistory', () => {
  ...
  it('should handle delete type with previous data', () => {
    ...
    expect(result.current.history[0].data.previousData).toEqual({
      name: 'D100',
      address: 'D100',
      enabled: true,
    });
