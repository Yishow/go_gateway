...
describe('legacyRoutes', () => {
  ...
  it('builds redirect target for legacy route', () => {
    expect(buildLegacyMigrationRedirect('points')).toBe(
      '/datalink?legacy=points&modal=points'
    );
    expect(buildLegacyMigrationRedirect('mappings')).toBe(
      '/datalink?legacy=mappings&modal=mappings'
    );
    expect(buildLegacyMigrationRedirect('wizard')).toBe(
      '/datalink?legacy=wizard&modal=wizard'
    );
  });
  ...
  it('builds redirect target for dashboard section', () => {
    expect(buildDashboardSectionRedirect('devices')).toBe(
      '/datalink?section=devices'
    );
    expect(buildDashboardSectionRedirect('settings')).toBe(
      '/datalink?section=settings'
    );
  });
  ...
  it('builds redirect target for dashboard modal intent', () => {
    ...
    expect(buildDashboardModalRedirect('polling-groups')).toBe(
      '/datalink?modal=polling-groups'
    );
