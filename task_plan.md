...
- [x] 以 TDD 補 `internal/datalink/mapping/sql_repo_test.go`，鎖住 nullable lifecycle 欄位的 `GetByID/List` 回歸測試
- [x] 修正 `internal/datalink/mapping/sql_repo.go` 的 nullable scan 路徑，完成 `woe-v2-mui` commit `ba75251`
- [x] 以 `agent-browser` 補齊 follow-up Shell / Output evidence
- [x] 同步 `openspec/.../tasks.md` 與本地 tracking records

**備註**：
- 這輪沒有再修改 Shell / Output wrapper UI；真正阻塞 browser gate 的是 shared backend mappings scan bug。
- follow-up evidence：
...
## 臨時任務：2026-04-09 Phase 6 v2 最終整體驗收（本輪）

- [x] 依 `6.1 共用基礎` 重跑同一條 `/studio` E2E：`Device -> Source -> Tag -> Output -> Shell`
- [x] 以真實裝置 `UI 4.3 Modbus TCP` 補齊 v2 final browser evidence
- [x] 驗證 Local Modbus 與 Database 兩個 target families 都仍落在同一個 incident target workspace
- [x] 修正 final validation 期間發現的 v2-specific a11y issue（database mapping selects 缺少 `name`）
- [x] 同步更新 branch `tasks.md`、`findings.md`、`progress.md`
...
- 本輪收尾後，下一個 fixed-cadence task 為 `6.1.v1 Linear Control Room`

---

## 臨時任務：2026-04-09 Phase 5 v2 Sentry Incident Desk（本輪）

- [x] 同步 Phase 5 shared contract 到 `woe-v2-mui`
- [x] 建立 incident strip，收斂 readiness summary、active blocker、diagnostics refresh、shortest return action
- [x] 補上 v2 shell red/green test、shell compare wrapper 與相容 testid 修補
- [x] 完成真機 evidence、型別檢查、targeted ESLint、targeted Vitest 與 frontend build
...
- canonical shell 差異點：
  - incident strip 直接在 command strip 上講 blocker 與 return path
  - shell refresh 成為一級 affordance，成功 / 失敗 / retry 都留在 shell surface
  - 既有 primary CTA 仍保留，代表 shell 新增的是 diagnostics / return layer，而不是吃掉 step-local flow
